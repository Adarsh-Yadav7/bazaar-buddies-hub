...
const SupplierDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-green-100 text-foreground font-inter">
      {/* Header */}
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-green-900">Supplier Dashboard</h1>
                <div className="flex items-center space-x-2">
                  <p className="text-sm text-green-600">Fresh Farm Co.</p>
                  <Badge className="bg-green-600 text-white"><CheckCircle className="mr-1 h-3 w-3" /> Verified</Badge>
                </div>
              </div>
            </div>
            <Button variant="outline" onClick={() => navigate('/')} className="hover:bg-red-50 text-red-600 border-red-200">
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card className="rounded-2xl shadow-md hover:shadow-lg transition-shadow">
              <CardHeader>
                <CardTitle className="flex items-center text-green-800"><TrendingUp className="mr-2 h-5 w-5 text-green-600" /> Your Reach</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="text-center">
                  <div className="text-3xl font-bold text-green-600">542</div>
                  <div className="text-sm text-muted-foreground">Active Vendors</div>
                </div>
                <div className="flex justify-between text-sm text-muted-foreground">
                  <span>This Month</span> <span className="font-semibold text-green-800">+48</span>
                </div>
                <div className="flex justify-between text-sm text-muted-foreground">
                  <span>Rating</span>
                  <div className="flex items-center">
            ...
            <Card className="rounded-2xl shadow-md hover:shadow-lg transition-shadow">
              <CardHeader>
                <CardTitle className="text-green-800">Trust Score</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex justify-between text-sm text-muted-foreground">
                  <span>Verification</span>
                ...
                <div className="flex justify-between text-sm text-muted-foreground">
                  <span>Trust Score</span> <span className="font-semibold text-green-700">95%</span>
                </div>
                <div className="flex justify-between text-sm text-muted-foreground">
                  <span>Reviews</span> <span className="font-semibold text-foreground">127</span>
                </div>
                <Button size="sm" className="w-full rounded-xl bg-green-600 text-white hover:bg-green-700">Boost Trust Score</Button>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-semibold text-green-800">Your Products</h2>
                <Button className="bg-green-600 hover:bg-green-700 text-white rounded-xl"><Plus className="mr-2 h-4 w-4" /> Add Product</Button>
              </div>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {products.map(product => (
                  <Card key={product.id} className="rounded-2xl shadow-md hover:shadow-xl transition-shadow">
                    <CardContent className="pt-6 space-y-4">
                      <div className="text-center">
                        <div className="text-4xl mb-2 animate-pulse">{product.image}</div>
            ...
            <Card className="rounded-2xl shadow-md">
              <CardHeader>
                <CardTitle className="flex items-center text-green-800"><Package className="mr-2 h-5 w-5" /> Real-time Order Management</CardTitle>
                <CardDescription>Manage and track all incoming orders from vendors</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {orders.map(order => (
                  <div key={order.id} className="flex justify-between items-center p-4 border rounded-xl bg-white/60 hover:bg-white/80 transition-colors">
                    <div>
                      <div className="flex items-center space-x-3">
                        <h4 className="font-semibold text-green-800">{order.vendor}</h4>
                        <Badge className={getStatusColor(order.status)}>{order.status}</Badge>
                      </div>
                      <p className="text-sm text-muted-foreground">{order.product} - {order.quantity}</p>
                      <div className="flex items-center text-xs text-muted-foreground mt-1">
                    ...
                    <div className="flex gap-2">
                      ...
                      {order.status === 'pending' && <Button size="sm" className="rounded-xl bg-green-600 text-white hover:bg-green-700">Accept</Button>}
                    </div>
                  </div>
                ))}
              </CardContent>
            ...
            <Card className="rounded-2xl shadow-md">
              ...
              <CardContent>
                <div className="grid md:grid-cols-4 gap-4">
                  <Button variant="outline" className="h-20 flex-col rounded-xl text-green-800 border-green-200 hover:bg-green-100"><Plus className="h-6 w-6 mb-2" /> <span>Add Product</span></Button>
                  <Button variant="outline" className="h-20 flex-col rounded-xl text-green-800 border-green-200 hover:bg-green-100"><Users className="h-6 w-6 mb-2" /> <span>View Vendors</span></Button>
                  <Button variant="outline" className="h-20 flex-col rounded-xl text-green-800 border-green-200 hover:bg-green-100"><Star className="h-6 w-6 mb-2" /> <span>Ratings</span></Button>
                  <Button variant="outline" className="h-20 flex-col rounded-xl text-green-800 border-green-200 hover:bg-green-100"><TrendingUp className="h-6 w-6 mb-2" /> <span>Analytics</span></Button>
                </div>
              </CardContent>
            </Card>
          </div>
